 ...
 * TransactionAttribute实现，通过应用大量回滚规则（正数和负数），确定给定异常是否应导致事务回滚。
 * If no rules are relevant to the exception, it
 * behaves like DefaultTransactionAttribute (rolling back on runtime exceptions).
 * 如果没有与异常相关的规则，则其行为类似于DefaultTransactionAttribute（在运行时异常时回滚）。
